...
\end{equation}
where $\mathbf{V}_q$ is a $p\times q$ matrix with $q$ orthogonal unit vectors as columns, $\lambda$ a $q$ vector of parameters and $\mu$ a location vector in $\CMcal{R}^p$. This is $p$ hyperplane. Fitting such a model corresponds to minimize the reconstruction error:
\begin{equation}
...
\begin{figure}
\centering
\includegraphics[scale=0.6]{img/SVD}
...
\caption{Equivalence of the non-parametric model using image representation.}
\label{modelImage}
\end{figure}
