...
Starting from the general continuous basis for five regions (four knots):
\begin{equation}
\begin{aligned}
...
&h_{12}(X) = X-\xi_3,\quad h_{13}(X) = (X-\xi_3)^2,\quad h_{14}(X) = (X-\xi_3)^3
\end{aligned}
\end{equation}
...
&h_5(X) = (X-\xi_0)^3, \quad h_6(X) = (X-\xi_1)^3,
\\&h_7(X) = (X-\xi_2)^3, \quad h_8(X) = (X-\xi_3)^3.
\end{aligned}
\end{equation}

Enforcing linearity beyond the boundaries, we eliminates $ h_3(X)$ and $ h_4(X)$. We must also keep the cubic expression in the intermediate regions but somehow make the cubic part cancel out when reaching the third region since here it must be linear. We can add for each region in the interval $[1, K-2]$ the following basis function:
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
Considering just the 3rd region for example:
\begin{equation}
\begin{aligned}
h_k(X) &= \frac{\br{X-\xi_0}_+^3 - \br{X-\xi_1}_+^3}{\xi_1 - \xi_0} =\\ &=\frac{-3\xi_0X^2+3\xi_0^2X-\xi_0^3+3\xi_1X^2-3\xi_1^2X-\xi_1^3}{\xi_1 - \xi_0}
\end{aligned}
\end{equation}
\end{document} 